...
class InstagramScraper:
    ...
    def __init__(self, username):
        ...
        self.future_to_item = {}
        self.MongoTools = MongoTools()
        self.lastMediaTime = self.MongoTools.getLastMediaTime(self.username)
    ...
    def crawl(self, max_id=None):
        ...
        for item in media['items']:
            future = self.executor.submit(self.download, item, './pics/' + self.username)
    ...
    def get_media(self, max_id):
        ...
        if resp.status_code == 200:
            ...
            if not media['items']:
                ...
                print('User %s is private' % self.username)

            return media
        else:
            # raise ValueError('User %s does not exist' % self.username)
            print('User %s does not exist' % self.username)

    def download(self, item, save_dir='./'):
        if int(item['created_time']) > int(self.lastMediaTime):
            media = {
                "code":item['code'],
                "location":item['location'],
                "low_resolution":item['images']['low_resolution']['url'].split('?')[0],
                "thumbnail":item['images']['thumbnail']['url'].split('?')[0],
                "standard_resolution":item['images']['standard_resolution']['url'].split('?')[0],
                "link":item['link'],
            ...
            self.MongoTools.insertMedia(media)
